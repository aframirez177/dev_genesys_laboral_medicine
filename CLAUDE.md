...
- URL: https://workflows.genesyslm.com.co/
- Hosted on DigitalOcean server (production only, not available locally)
- Purpose: Automation workflows, AI integrations, external API orchestration

**Available Integrations**:
- OpenAI API - For AI-powered suggestions and intelligence
- Google API - For Google services integration
- Meta WhatsApp API - For WhatsApp notifications and communications
- N8N Workflows - Custom automation pipelines

**Use Cases**:
- AI-powered risk suggestions in wizard (can call OpenAI via N8N)
- Automated notifications via WhatsApp for exam expiration alerts
...
**Integration Pattern**:
```javascript
// Backend can trigger N8N workflows via webhook
const triggerWorkflow = async (workflowName, data) => {
  const response = await fetch(`https://workflows.genesyslm.com.co/webhook/${workflowName}`, {
    method: 'POST',
...
**Note**: N8N is only available in production environment, not in local development.
