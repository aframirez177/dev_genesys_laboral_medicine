...
async function generarMatrizExcel(
  ...
  const columnasCompletas = [
    ...
    {
      header: "Nivel de deficiencia",
      key: "nd",
      width: 10,
      style: { alignment: { horizontal: "center" } },
    },
    {
      header: "Nivel de exposición",
      key: "ne",
      width: 10,
      style: { alignment: { horizontal: "center" } },
    },
    {
      header: "Nivel de probabilidad (NP)",
      key: "np_valor",
      width: 12,
      style: { alignment: { horizontal: "center" } },
    },
    {
      header: "Nivel de Probabilidad (Categoría)",
      key: "np_nivel_categoria",
      width: 18,
      style: { alignment: { horizontal: "center" } },
    },
    {
      header: "Interpretación del nivel de probabilidad",
      key: "np_interpretacion",
      width: 30,
    },
    {
      header: "Nivel de consecuencia",
      key: "nc_valor",
      width: 10,
      style: { alignment: { horizontal: "center" } },
    },
    {
      header: "Nivel de riesgo (NR) e intervención",
      key: "nr_valor_intervencion",
      width: 12,
      style: { alignment: { horizontal: "center" } },
    },
    {
      header: "Nivel de Riesgo (Categoría)",
      key: "nr_interpretacion_nivel",
      width: 15,
      style: { alignment: { horizontal: "center" } },
    },
    {
      header: "Interpretación del NR",
      key: "nr_interpretacion_texto",
      width: 35,
    },
    {
      header: "Aceptabilidad del riesgo",
      key: "aceptabilidad_riesgo",
      width: 30,
    },
    {
      header: "Nro. Expuestos",
      key: "nro_expuestos",
      width: 10,
      style: { alignment: { horizontal: "center" } },
    },
    ...
    {
      header: "Existencia requisito legal específico asociado (Si o No)",
      key: "requisito_legal",
      width: 18,
      style: { alignment: { horizontal: "center" } },
    },
    ...
    {
      header: "Controles de ingeniería",
      key: "medida_ctrl_ingenieria",
      width: 35,
    },
    {
      header: "Controles administrativos, Señalización, Advertencia",
      key: "medida_ctrl_admin",
      width: 40,
    },
    {
      header: "Equipos/ Elementos de protección personal",
      key: "epp",
      width: 40,
    },
  ];
  ...
  for (const procesoNombre in cargosPorProceso) {
    ...
    cargosEnEsteProceso.forEach((cargo) => {
      ...
      const gesAgrupadosPorClasificacion = gesSeleccionados.reduce(
        (acc, ges) => {
      ...
      for (const clasificacionKey in gesAgrupadosPorClasificacion) {
        const gesDeMismaClasificacion =
          gesAgrupadosPorClasificacion[clasificacionKey];
        ...
        gesDeMismaClasificacion.forEach((ges) => {
          ...
          try {
            ...
            console.error("Error cálculo niveles:", e);
          }
          ...
          const rowData = {
            ...
            efectos_posibles:
              datosGesPredefinidos.consecuencias || "No especificado",
            ...
            peor_consecuencia:
              datosGesPredefinidos.peorConsecuencia || "No especificado",
            requisito_legal: "Si",
            medida_eliminacion:
              datosGesPredefinidos.medidasIntervencion?.eliminacion || "",
            medida_sustitucion:
              datosGesPredefinidos.medidasIntervencion?.sustitucion || "",
            medida_ctrl_ingenieria:
              datosGesPredefinidos.medidasIntervencion?.controlesIngenieria ||
              "",
            medida_ctrl_admin:
              datosGesPredefinidos.medidasIntervencion
          ...
          if (!isFree) {
            ...
            if (coloresNiveles.exposicion[rowData.ne])
              neCell.fill = {
                type: "pattern",
                pattern: "solid",
                fgColor: { argb: coloresNiveles.exposicion[rowData.ne] },
              };
            if (coloresNiveles.consecuencia[rowData.nc_valor])
              ncCell.fill = {
                type: "pattern",
                pattern: "solid",
                fgColor: {
                  argb: coloresNiveles.consecuencia[rowData.nc_valor],
                },
              };

            const npCategoriaCell = addedRow.getCell("np_nivel_categoria");
            if (coloresNivelProbabilidadCategoria[nivelProb.nivel])
              npCategoriaCell.fill = {
                type: "pattern",
                pattern: "solid",
                fgColor: {
                  argb: coloresNivelProbabilidadCategoria[nivelProb.nivel],
                },
              };
            ...
            if (nivelRiesgo.color) {
              const bgColorHex = nivelRiesgo.color
                .replace("#", "")
                .toUpperCase();
              const cellFill = {
                type: "pattern",
          ...
          addedRow.eachCell({ includeEmpty: true }, (cell, colNumber) => {
            ...
            const centeredKeys = [
              ...
              "np_nivel_categoria",
              "nc_valor",
              "nr_valor_intervencion",
              "nr_interpretacion_nivel",
              "nro_expuestos",
              "requisito_legal",
            ];
        ...
        if (!isFree && gesDeMismaClasificacion.length > 1) {
          ...
          // AGREGAMOS VALIDACIÓN AQUÍ ⬇️
          if (endRowForClasificacion > startRowForClasificacion) {
            worksheet.mergeCells(
              startRowForClasificacion,
              worksheet.getColumn("peligro_clasificacion").number,
              endRowForClasificacion,
              worksheet.getColumn("peligro_clasificacion").number
            );
          }
          worksheet.mergeCells(
          ...
          const cellClasificacion = worksheet.getCell(
            startRowForClasificacion,
            worksheet.getColumn("peligro_clasificacion").number
          );
          cellClasificacion.alignment = {
            vertical: "top",
            horizontal: "left",
            wrapText: true,
          };
      ...
      if (
        !isFree &&
        endRowForCurrentCargo >= startRowForCurrentCargo &&
        gesSeleccionados.length > 0
      ) {
        const keysCargoComun = [
          "zona_lugar",
          "actividades",
          "tareas",
          "rutinario",
        ...
        keysCargoComun.forEach((key) => {
          ...
          if (endRowForCurrentCargo > startRowForCurrentCargo) {
            worksheet.mergeCells(
              startRowForCurrentCargo,
              colNum,
              endRowForCurrentCargo,
              colNum
            );
          }
    ...
    if (
      !isFree &&
      finFilasProceso >= inicioFilasProceso &&
      finFilasProceso > inicioFilasProceso
    ) {
      worksheet.mergeCells(
        inicioFilasProceso,
        worksheet.getColumn("proceso").number,
        finFilasProceso,
        worksheet.getColumn("proceso").number
      );
      const cellProceso = worksheet.getCell(
        inicioFilasProceso,
        worksheet.getColumn("proceso").number
      );
      cellProceso.alignment = {
        vertical: "top",
        horizontal: "left",
        wrapText: true,
      };
      ...
      !isFree &&
      finFilasProceso === inicioFilasProceso &&
      cargosEnEsteProceso.length > 0 &&
      cargosEnEsteProceso[0].gesSeleccionados.length > 0
    ) {
      const cellProceso = worksheet.getCell(
        inicioFilasProceso,
        worksheet.getColumn("proceso").number
