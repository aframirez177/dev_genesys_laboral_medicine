...
export function initializeForm() {
  ...
  let tutorialMostradoOriginalmente =
    localStorage.getItem("tutorialMatrizRiesgosMostrado_v1") === "true";
  ...
  try {
    ...
    if (savedHistorical) {
      ...
      if (parsed.controles) {
        historicalValues.controles.fuente = new Set(
          parsed.controles.fuente || []
        );
        historicalValues.controles.medio = new Set(
          parsed.controles.medio || []
        );
        historicalValues.controles.individuo = new Set(
          parsed.controles.individuo || []
        );
      }
    }
  } catch (error) {
  ...
  function gatherFormData() {
    ...
    cargoDivs.forEach((cargoDiv, index) => {
      ...
      const numTrabajadoresEl = cargoDiv.querySelector(
        'input[name="numTrabajadores"]'
      );
      const descripcionTareasEl = cargoDiv.querySelector(
        'textarea[name="descripcionTareas"]'
      );
      ...
      console.log(
        `Cargo ${index + 1} (${
          cargoData.cargoName
        }): Buscando GES seleccionados...`
      );

      cargoDiv
        .querySelectorAll('input[type="checkbox"][name^="ges_cargo_"]:checked')
        .forEach((checkbox) => {
          ...
          try {
            const controlesData = {
              fuente:
                cargoDiv.querySelector(
                  `input[type="hidden"][data-riesgo="${riesgoValue}"][data-tipo="fuente"]`
                )?.value || "Ninguno",
              medio:
                cargoDiv.querySelector(
                  `input[type="hidden"][data-riesgo="${riesgoValue}"][data-tipo="medio"]`
                )?.value || "Ninguno",
              individuo:
                cargoDiv.querySelector(
                  `input[type="hidden"][data-riesgo="${riesgoValue}"][data-tipo="individuo"]`
                )?.value || "Ninguno",
            };

            const nivelesInput = cargoDiv.querySelector(
              `[data-riesgo="${riesgoValue}"][data-niveles]`
            );
            let niveles = {};
            if (nivelesInput && nivelesInput.value) {
              console.log(
                `    Niveles encontrados para ${riesgoValue}:`,
                nivelesInput.value
              );
              try {
                ...
                console.error(
                  `    Error al parsear niveles para ${riesgoValue}:`,
                  e
                );
                niveles = {};
              }
      ...
      console.log(
        `Cargo ${index + 1} completado con ${
          cargoData.gesSeleccionados.length
        } GES`
      );
  ...
  function saveData() {
    ...
    cargos.forEach((cargo, index) => {
      const cargoName =
        cargo.querySelector('input[name="cargoName"]')?.value || "Sin nombre";
      const nivelesInputs = cargo.querySelectorAll("[data-niveles]");
      console.log(
        `Cargo ${index + 1} (${cargoName}): ${
          nivelesInputs.length
        } inputs de niveles en DOM`
      );
    ...
    if (
      ...
      !document.querySelector(".restore-banner")
    ) {
      return;
    }
    const saveObject = {
    ...
    try {
      ...
      localStorage.setItem(
        "historicalValues",
        JSON.stringify(historicalToSave)
      );
    } catch (error) {
  ...
  function validateCargosData() {
    ...
    cargoDivs.forEach((cargoDiv) => {
      ...
      const trabajadoresInput = cargoDiv.querySelector(
        'input[name="numTrabajadores"]'
      );
      const zonaInput = cargoDiv.querySelector('input[name="zona"]');

      if (!cargoNameInput || !cargoNameInput.value.trim()) {
        alert("Por favor, ingrese el nombre de todos los cargos.");
        isValid = false;
        return;
      }
      if (!areaInput || !areaInput.value.trim()) {
        alert(
          "Por favor, ingrese el área para el cargo: " +
            (cargoNameInput ? cargoNameInput.value : "desconocido")
        );
        isValid = false;
        return;
      }
      if (
        !trabajadoresInput ||
        !trabajadoresInput.value ||
        parseInt(trabajadoresInput.value) < 1
      ) {
        alert(
          "Por favor, ingrese un número válido de trabajadores para el cargo: " +
            (cargoNameInput ? cargoNameInput.value : "desconocido")
        );
        isValid = false;
        return;
      }
      if (!zonaInput || !zonaInput.value.trim()) {
        alert(
          "Por favor, ingrese la zona para el cargo: " +
            (cargoNameInput ? cargoNameInput.value : "desconocido")
        );
        isValid = false;
        return;
      ...
      const gesCheckboxes = cargoDiv.querySelectorAll(
        'input[type="checkbox"][name^="ges_cargo_"]:checked'
      ); // Ajustado
      if (gesCheckboxes.length === 0) {
        alert(
          `Debe seleccionar al menos un GES para el cargo '${
            cargoNameInput ? cargoNameInput.value : "desconocido"
          }'.`
        );
        isValid = false;
      ...
      gesCheckboxes.forEach((checkbox) => {
        ...
        if (!nivelesInput || !nivelesInput.value) {
          gesIncompletos.push({
            cargo: cargoNameInput ? cargoNameInput.value : "desconocido",
            ges: riesgoValue,
          });
          isValid = false;
          return;
        ...
        try {
          ...
          if (
            niveles.deficiencia === undefined ||
            niveles.exposicion === undefined ||
            niveles.consecuencia === undefined ||
            niveles.deficiencia.value === undefined ||
            niveles.exposicion.value === undefined ||
            niveles.consecuencia.value === undefined
          ) {
            gesIncompletos.push({
              cargo: cargoNameInput ? cargoNameInput.value : "desconocido",
              ges: riesgoValue,
    ...
    if (!isValid && gesIncompletos.length > 0) {
      let mensaje =
        "Debe completar todos los niveles (deficiencia, exposición y consecuencia) para los siguientes GES:\n\n";
  ...
  function updateNiveles(riesgoValue, cargoDiv, niveles) {
    ...
    if (!infoGeneralSection) {
      console.error(
        `No se encontró info-general-section en el cargo para ${riesgoValue}`
      );
      return;
    }

    let nivelesInput = cargoDiv.querySelector(
      `[data-riesgo="${riesgoValue}"][data-niveles]`
    );
    if (!nivelesInput) {
      ...
      nivelesInput.setAttribute(
        "name",
        `niveles_${riesgoValue.replace(/[^a-zA-Z0-9]/g, "_")}`
      );

      // AGREGAR AL FINAL DE LA SECCIÓN DE INFO GENERAL
      infoGeneralSection.appendChild(nivelesInput);
      console.log(
        `Creado nuevo input hidden para ${riesgoValue} en info-general-section`
      );
    } else {
    ...
    const verificacionInmediata = cargoDiv.querySelector(
      `[data-riesgo="${riesgoValue}"][data-niveles]`
    );
    if (verificacionInmediata) {
      console.log(
        `✅ Input verificado inmediatamente en DOM:`,
        verificacionInmediata.value
      );
    } else {
    ...
    setTimeout(() => {
      ...
      if (savedData) {
        try {
          ...
          console.log(
            `Verificación de persistencia para ${riesgoValue}:`,
            parsed
          );

          // Buscar si nuestros niveles están en los datos guardados
          const cargoGuardado = parsed.cargos.find((cargo) =>
            cargo.gesSeleccionados.some(
              (ges) => `${ges.riesgo} - ${ges.ges}` === riesgoValue
            )
          );

          if (cargoGuardado) {
            const gesGuardado = cargoGuardado.gesSeleccionados.find(
              (ges) => `${ges.riesgo} - ${ges.ges}` === riesgoValue
            );
            console.log(
              `Niveles persistidos en localStorage:`,
              gesGuardado?.niveles
            );
          } else {
            console.warn(
              `No se encontraron niveles persistidos para ${riesgoValue}`
            );
          }
        } catch (e) {
  ...
  function updateControles(riesgoValue, cargoDiv, tipo, valor) {
    ...
    if (!infoGeneralSection) {
      console.error(
        `No se encontró info-general-section en el cargo para control ${tipo} de ${riesgoValue}`
      );
      return;
    }

    // Buscar o crear el input hidden para este control específico
    let controlInput = cargoDiv.querySelector(
    ...
    if (!controlInput) {
      ...
      controlInput.setAttribute(
        "name",
        `control_${tipo}_${riesgoValue.replace(/[^a-zA-Z0-9]/g, "_")}`
      );
  ...
  function generateNivelesHTML(riesgoValue, cargoDiv) {
    function getNivelSeleccionado(riesgoVal, nivelNom, val, cDiv) {
      console.log(
        `🔍 getNivelSeleccionado: Buscando ${nivelNom}=${val} para ${riesgoVal}`
      );

      const nivelesData = cDiv.querySelector(
        `[data-riesgo="${riesgoVal}"][data-niveles]`
      );
      if (!nivelesData || !nivelesData.value) {
        console.log(
          `❌ No se encontraron datos de niveles para ${riesgoVal} - ${nivelNom}`
        );

        // DEBUGGING ADICIONAL: Ver qué inputs SÍ existen
        const todosLosInputsNiveles = cDiv.querySelectorAll("[data-niveles]");
        console.log(
          `   Inputs de niveles disponibles (${todosLosInputsNiveles.length}):`
        );
      ...
      try {
        ...
        console.log(
          `🔍 Verificando ${nivelNom} con valor ${val}:`,
          niveles[nivelNom]
        );
    ...
    const nivelesConfig = [
        ...
        opciones: [
          {
            label: "Bajo (B)",
            value: 0,
            d: "No se ha detectado consecuencia alguna, o la eficacia del conjunto de medidas preventivas existentes es alta.",
            c: "#4caf50",
            tooltipKey: "bajo",
          },
          {
            label: "Medio (M)",
            value: 2,
            d: "Se han detectado peligros que pueden dar lugar a consecuencias poco significativas.",
            c: "#ffeb3b",
            tooltipKey: "medio",
          },
          {
            label: "Alto (A)",
            value: 6,
            d: "Se ha(n) detectado algún(os) peligro(s) que pueden dar lugar a consecuencias significativa(s).",
            c: "#ff9800",
            tooltipKey: "alto",
          },
          {
            label: "Muy Alto (MA)",
            value: 10,
            d: "Se han detectado peligros que determinan como muy posible la generación de incidentes.",
            c: "#f44336",
            tooltipKey: "muyAlto",
          },
        ],
      },
      {
        ...
        descripcion:
          "Evalúa la frecuencia y duración de la exposición al riesgo",
        opciones: [
          {
            label: "Esporádica (EE)",
            value: 1,
            d: "La situación de exposición se presenta de manera eventual.",
            c: "#4caf50",
            tooltipKey: "esporadica",
          },
          {
            label: "Ocasional (EO)",
            value: 2,
            d: "La situación de exposición se presenta alguna vez durante la jornada laboral y por un periodo de tiempo corto.",
            c: "#ffeb3b",
            tooltipKey: "ocasional",
          },
          {
            label: "Frecuente (EF)",
            value: 3,
            d: "La situación de exposición se presenta varias veces durante la jornada laboral por tiempos cortos.",
            c: "#ff9800",
            tooltipKey: "frecuente",
          },
          {
            label: "Continua (EC)",
            value: 4,
            d: "La situación de exposición se presenta sin interrupción o varias veces con tiempo prolongado durante la jornada laboral.",
            c: "#f44336",
            tooltipKey: "continua",
          },
        ],
      },
      {
        ...
        opciones: [
          ...
          },
          {
            label: "Grave (G)",
            value: 25,
            d: "Lesiones o enfermedades con incapacidad laboral temporal.",
            c: "#ffeb3b",
            tooltipKey: "grave",
          },
          {
            label: "Muy Grave (MG)",
            value: 60,
            d: "Lesiones o enfermedades graves irreparables.",
            c: "#ff9800",
            tooltipKey: "muyGrave",
          },
          {
    ...
    const htmlGenerado = nivelesConfig
      .map((nivel) => {
        console.log(`🏗️ Generando HTML para nivel: ${nivel.nombre}`);

        const opcionesHTML = nivel.opciones
          .map((op) => {
            const isSelected = getNivelSeleccionado(
              riesgoValue,
              nivel.nombre,
              op.value,
              cargoDiv
            ...
            console.log(
              `   Opción ${op.label} (valor ${op.value}): selected = ${isSelected}`
            );
  ...
  function showControlesPopup(riesgoValue, cargoDiv, checkbox) {
    ...
    console.log(
      `🔢 Inputs de niveles en cargoDiv: ${nivelesEnCargoDiv.length}`
    );
    ...
    const inputEspecifico = cargoDiv.querySelector(
      `[data-riesgo="${riesgoValue}"][data-niveles]`
    );
    ...
    popup.innerHTML = `
            ...
            <div class="control-group">
                ...
                <div class="niveles-section"><h5>Evaluación de Niveles</h5>${generateNivelesHTML(
                  riesgoValue,
                  cargoDiv
                )}</div>
            </div>`;
    ...
    if (
      left < 0 &&
      checkboxRect.right + 10 + popupRect.width <= window.innerWidth
    ) {
    ...
    const repositionOnScroll = () => {
      ...
      let newTop =
        newCheckboxRect.top +
        newCheckboxRect.height / 2 -
        newPopupRect.height / 2;
      let newLeft = newCheckboxRect.right + 10;

      if (newLeft + newPopupRect.width > window.innerWidth)
        newLeft = newCheckboxRect.left - newPopupRect.width - 10;
      if (newTop + newPopupRect.height > window.innerHeight)
        newTop = window.innerHeight - newPopupRect.height - 10;
      if (newTop < 0) newTop = 10;
      if (
        newLeft < 0 &&
        newCheckboxRect.right + 10 + newPopupRect.width <= window.innerWidth
      ) {
    ...
    popup.querySelector(".close-popup").onclick = () => {
      ...
      setTimeout(() => {
        const verificacionFinal = cargoDiv.querySelector(
          `[data-riesgo="${riesgoValue}"][data-niveles]`
        );
        console.log(
          `Verificación final post-cierre para ${riesgoValue}:`,
    ...
    popup.querySelectorAll(".control-group input").forEach((input) => {
      ...
      input.oninput = () => {
        ...
        updateDatalist(
          input,
          historicalValues.controles[tipo],
          `${tipo}-datalist`
        );
        updateControles(riesgoValue, cargoDiv, tipo, valor);
      };
    ...
    popup.querySelectorAll(".barra").forEach((barra) => {
      ...
      barra.onclick = () => {
        ...
        nivelDiv
          .querySelectorAll(".barra")
          .forEach((b) => b.classList.remove("selected"));
        barra.classList.add("selected");

        const nivelesData = cargoDiv.querySelector(
          `[data-riesgo="${riesgoValue}"][data-niveles]`
        );
        const niveles =
          nivelesData && nivelesData.value ? JSON.parse(nivelesData.value) : {};
        ...
        console.log(
          `Actualizando nivel ${nivelName} para ${riesgoValue}:`,
          niveles[nivelName]
        );
    ...
    popup.querySelectorAll(".info-icon").forEach((icon) => {
      icon.onmouseenter = () => {
        ...
        switch (nivelType) {
          case "deficiencia":
            description =
              "Evalúa qué tan efectivos son los controles existentes para prevenir o reducir el riesgo.";
            break;
          case "exposicion":
            description =
              "Mide con qué frecuencia y por cuánto tiempo los trabajadores están expuestos al riesgo.";
            break;
          case "consecuencia":
            description =
              "Determina qué tan grave puede ser el daño si el riesgo se materializa.";
            break;
        }
      ...
      switch (tipo) {
        case "fuente":
          tooltipInfo = {
            ...
            description:
              "Medidas que eliminan o reducen el peligro directamente en su origen.",
        ...
        case "medio":
          tooltipInfo = {
            ...
            description:
              "Medidas que actúan sobre el ambiente de trabajo para reducir la exposición.",
        ...
        case "individuo":
          tooltipInfo = {
            ...
            description:
              "Medidas que protegen directamente al trabajador expuesto.",
    ...
    if (selectedGes.length > 0) {
      selectedGes.forEach((ges) => {
        ...
        if (nivelesInput) {
          ...
          const todosLosInputsNiveles =
            cargoDiv.querySelectorAll("[data-niveles]");
          console.log(
            `   🔍 Inputs de niveles disponibles (${todosLosInputsNiveles.length}):`
          );
          todosLosInputsNiveles.forEach((input) => {
        ...
        if (nivelesInput && nivelesInput.value) {
          try {
            ...
            console.log(
              `     - deficiencia: ${niveles.deficiencia?.value !== undefined}`
            );
            console.log(
              `     - exposicion: ${niveles.exposicion?.value !== undefined}`
            );
            console.log(
              `     - consecuencia: ${
                niveles.consecuencia?.value !== undefined
        ...
        checkMark.className = `check-mark ${
          nivelesCompletos ? "complete" : "incomplete"
        }`;
        checkMark.textContent = nivelesCompletos ? "✓" : "!";

        console.log(
          `   🎨 Creando icono: ${
            nivelesCompletos ? "✓ (completo)" : "! (incompleto)"
          }`
        );
        ...
        gesItem.className = `ges-resumen-item ${
          nivelesCompletos ? "complete-ges" : "incomplete-ges"
        }`;
        gesItem.title = nivelesCompletos
          ? "Niveles completos"
          : "Falta completar los niveles de este GES";
  ...
  function iniciarTutorialPrimerCargo(cargoDiv) {
    if (
      tutorialMostradoOriginalmente ||
      tutorialActualmenteActivo ||
      !cargoDiv
    ) {
    ...
    const pasosTutorial = [
      {
        ...
        mensaje:
          "Comienza ingresando el nombre del cargo, por ejemplo, 'Secretaria Administrativa' o 'Operario de Máquina'.",
        posicion: "bottom",
      },
      {
        ...
        mensaje:
          "Indica cuántos trabajadores desempeñan este cargo. Puedes escribir directamente con el teclado o usar los botones '+' y '-' para ajustar el número.",
      ...
      },
      {
        ...
        mensaje:
          "Indica el área o departamento al que pertenece este cargo, como 'Administración' o 'Producción'.",
        posicion: "bottom",
      },
      {
        ...
        mensaje:
          "Especifica la zona o lugar físico donde se desempeña principalmente este cargo.",
        posicion: "bottom",
      },
      {
        ...
        mensaje:
          "Describe brevemente las principales tareas y responsabilidades de este cargo.",
        posicion: "top",
      },
      {
        ...
        mensaje:
          "Usa estos interruptores para indicar características especiales del cargo, como si manipula alimentos o trabaja en alturas.",
        posicion: "bottom",
      },
      {
        ...
        mensaje:
          "Explora los diferentes tipos de riesgos. Selecciona los GES (Grupos de Exposición Similar) que apliquen a este cargo.",
        posicion: "bottom",
      },
      {
        selector:
          ".riesgos-swiper .swiper-slide:first-child .ges-list .ges-item:first-child label",
        elementoFriendlyName: "Checkbox del Primer GES",
        mensaje:
          "Haz clic en un GES para seleccionarlo. Luego, podrás definir los controles existentes y evaluar los niveles de riesgo al hacer clic en el nombre del GES en el resumen de abajo.",
        posicion: "right",
      },
      {
        ...
        mensaje:
          "Aquí verás los GES seleccionados. Haz clic en cada uno para detallar los controles y niveles. ¡Un check verde indica que está completo!",
        posicion: "top",
    ...
    function posicionarPopup(popup, targetElement, posicion = "bottom") {
      ...
      popup.style.top = `${Math.max(
        minMargin,
        Math.min(top, window.innerHeight - popupRect.height - minMargin)
      )}px`;
      popup.style.left = `${Math.max(
        minMargin,
        Math.min(left, window.innerWidth - popupRect.width - minMargin)
      )}px`;
    }
    ...
    function mostrarPaso(indicePaso) {
      ...
      if (!elementoTarget) {
        console.warn(
          `Tutorial: Elemento no encontrado para '${paso.elementoFriendlyName}' (selector: ${paso.selector})`
        );
        pasoActualIdx++;
        mostrarPaso(pasoActualIdx);
        return;
      }
      ...
      const scrollEndListener = () => {
        ...
        if (clasePosicionFlecha)
          popup.classList.add(`pos-${clasePosicionFlecha}`);
        ...
        popup.innerHTML = `
                    ...
                    <div class="tutorial-acciones">
                        ${
                          indicePaso > 0
                            ? '<button class="tutorial-btn-anterior" type="button">Anterior</button>'
                            : ""
                        }
                        <button class="tutorial-btn-siguiente" type="button">${
                          indicePaso === pasosTutorial.length - 1
                            ? "Finalizar Tutorial"
                            : "Siguiente"
                        }</button>
        ...
        popup.querySelector(".tutorial-btn-siguiente").onclick = () => {
          if (paso.esUltimoPasoConTarget)
            elementoTarget.classList.remove("tutorial-highlight");
          pasoActualIdx++;
          mostrarPaso(pasoActualIdx);
        };
        if (indicePaso > 0) {
  ...
  function addCargo(cargoData = {}, isDefault = false) {
    ...
    cargoTitle.textContent =
      cargoData.cargoName || `Cargo #${currentCargoCount + 1}`;
      ...
      if (cargoContainer.querySelectorAll(".cargo").length > 1) {
        if (confirm("¿Está seguro de que desea eliminar este cargo?")) {
          ...
          if (remainingCargos.length === 1) {
            const lastDeleteBtn =
              remainingCargos[0].querySelector(".delete-btn");
    ...
    cargoNameInput.oninput = () => {
      cargoTitle.textContent =
        cargoNameInput.value ||
        `Cargo #${cargoContainer.querySelectorAll(".cargo").length}`;
    };
    cargoNameInput.onblur = () => {
    ...
    tareasTextarea.oninput = debounce(saveData, 300);
    infoGeneralSection.append(
      cargoNameInput,
      areaInput,
      zonaInput,
      tareasTextarea
    ...
    swiperContainerEl.className = "swiper riesgos-swiper";
    ...
    riesgos.forEach((riesgo) => {
      ...
      swiperSlide.style.backgroundColor = riesgoColors[riesgo] || "#f0f0f0";
      const slideContent = document.createElement("div");
      slideContent.className = "slide-content";
      const riesgoTitleH3 = document.createElement("h3");
      riesgoTitleH3.textContent = `Riesgo ${riesgo}`;
      const gesList = getGesListByRiesgo(riesgo);
      const gesListDiv = document.createElement("div");
      gesListDiv.className = "ges-list";
      gesList.forEach((ges) => {
        const gesItemDiv = document.createElement("div");
        gesItemDiv.className = "ges-item";
        const gesCheckbox = document.createElement("input");
        gesCheckbox.type = "checkbox";
        const gesNameUnico = `ges_cargo_${uniqueCargoIdSuffix}_${riesgo.replace(
          /[^a-zA-Z0-9]/g,
          ""
        )}_${ges.replace(/[^a-zA-Z0-9]/g, "")}`;
        gesCheckbox.name = gesNameUnico;
        gesCheckbox.id = gesNameUnico;
        gesCheckbox.value = `${riesgo} - ${ges}`;

        if (
          cargoData.gesSeleccionados &&
          cargoData.gesSeleccionados.find(
            (g) => g.riesgo === riesgo && g.ges === ges
          )
        ) {
        ...
        gesCheckbox.onchange = () => {
          if (gesCheckbox.checked)
            ...
            if (
              existingPopup &&
              existingPopup.querySelector("h4")?.textContent === ges
            ) {
    ...
    if (
      esPrimerCargoAgregado &&
      !tutorialMostradoOriginalmente &&
      cargoDiv.isConnected
    ) {
      ...
      if (primerCargoDiv) {
        ...
        setTimeout(() => {
          if (
            !tutorialActualmenteActivo &&
            !tutorialMostradoOriginalmente &&
            primerCargoDiv.offsetParent !== null
          ) {
  ...
  function showRestoreBanner(state) {
    ...
    const container =
      document.querySelector(".matriz-riesgos-section") || document.body;
  ...
  function restoreFormState(state) {
    try {
      if (!cargoContainer) {
        console.error(
          "Error crítico: cargoContainer no encontrado al restaurar."
        );
        return;
      }

      // Limpiar solo los cargos existentes, no todo el contenedor
      const cargosToRemove = cargoContainer.querySelectorAll(".cargo");
      cargosToRemove.forEach((cargo) => cargo.remove());
      console.log(
        `Eliminados ${cargosToRemove.length} cargos existentes antes de restaurar.`
      );

      esPrimerCargoAgregado = state.cargos.length <= 1;
      console.log(
        `Restaurando ${state.cargos.length} cargos desde localStorage...`
      );

      // Recorrer los datos guardados
      state.cargos.forEach((cargoData, index) => {
        const currentCargoNameForLog =
          cargoData.cargoName || `(Cargo sin nombre ${index + 1})`; // Para logs claros
        console.log(
          `Restaurando Cargo ${index + 1}: ${currentCargoNameForLog}`
        );

        // Llamar a addCargo UNA SOLA VEZ y guardar la referencia
        const cargoEl = addCargo(
          cargoData,
          index === 0 && state.cargos.length === 1
        );

        if (!cargoEl || !cargoEl.isConnected) {
          // Verificar si se creó Y está en el DOM
          console.error(
            `Error: addCargo no creó correctamente el elemento DOM para ${currentCargoNameForLog}. Saltando este cargo.`
          );
          return;
        }
        console.log(
          `  Elemento DOM para ${currentCargoNameForLog} creado y conectado.`
        );
        // Log para verificar que cargoEl es el correcto
        console.log(
          `  Verificación: Nombre en cargoEl recién creado: ${
            cargoEl.querySelector('input[name="cargoName"]')?.value
          }`
        );
        ...
        if (
          cargoData.gesSeleccionados &&
          cargoData.gesSeleccionados.length > 0
        ) {
          console.log(
            `  Restaurando ${cargoData.gesSeleccionados.length} GES para ${currentCargoNameForLog}...`
          );
          cargoData.gesSeleccionados.forEach((gesData) => {
            const gesValue = `${gesData.riesgo} - ${gesData.ges}`;
            const checkbox = cargoEl.querySelector(
              `input[type="checkbox"][value="${gesValue}"]`
            );
            ...
            if (checkbox) {
              ...
              if (gesData.controles) {
                ["fuente", "medio", "individuo"].forEach((tipo) => {
                  ...
                  if (valorGuardado !== undefined && valorGuardado !== null) {
                    console.log(
                      `    Procesando control ${tipo} = "${valorGuardado}" para ${gesValue}`
                    );
                    const infoGeneralSection = cargoEl.querySelector(
                      ".info-general-section"
                    );

                    if (infoGeneralSection) {
                      let controlInput = cargoEl.querySelector(
                        `input[type="hidden"][data-riesgo="${gesValue}"][data-tipo="${tipo}"]`
                      );
                      ...
                      if (!controlInput) {
                        ...
                        controlInput.setAttribute(
                          "name",
                          `control_${tipo}_${gesValue.replace(
                            /[^a-zA-Z0-9]/g,
                            "_"
                          )}`
                        );
                        ...
                        console.log(
                          `      Input hidden PREPARADO para control ${tipo}: ${gesValue}`
                        );
                      } else {
                        console.log(
                          `      Input hidden ENCONTRADO para control ${tipo}: ${gesValue}`
                        );
                      }
                      ...
                      console.log(
                        `      >> Valor ASIGNADO a input (${tipo}): ${controlInput.value}`
                      );
                      ...
                      if (needsAppend) {
                        infoGeneralSection.appendChild(controlInput);
                        console.log(
                          `      Input hidden AÑADIDO al DOM para control ${tipo}`
                        );
                      }

                      // VERIFICACIÓN INMEDIATA después de asignar/añadir
                      const verifyInput = infoGeneralSection.querySelector(
                        `input[type="hidden"][data-riesgo="${gesValue}"][data-tipo="${tipo}"]`
                      );
                      if (verifyInput && verifyInput.value === valorGuardado) {
                        console.log(
                          `      ✅ Verificación INMEDIATA OK para ${tipo}. Valor en DOM: "${verifyInput.value}"`
                        );
                      } else {
                        console.error(
                          `      ❌ Verificación INMEDIATA FALLÓ para ${tipo}. Input:`,
                          verifyInput
                        );
                      }
                    } else {
                      // Fin if(infoGeneralSection)
                      console.error(
                        `      Error: No se encontró .info-general-section para control ${tipo} en ${gesValue} DENTRO del cargo ${currentCargoNameForLog}`
                      );
                    }
                  } // Fin if(valorGuardado !== undefined)
                }); // Fin forEach tipo control
              } else {
                // Fin if(gesData.controles)
              ...
              if (gesData.niveles && Object.keys(gesData.niveles).length > 0) {
                ...
                console.log(
                  `    Restaurando niveles JSON = ${valorJsonGuardado} para ${gesValue}`
                );
                const infoGeneralSection = cargoEl.querySelector(
                  ".info-general-section"
                );
                ...
                if (infoGeneralSection) {
                  let nivelesInput = cargoEl.querySelector(
                    `input[type="hidden"][data-riesgo="${gesValue}"][data-niveles]`
                  );
                  ...
                  if (!nivelesInput) {
                    ...
                    nivelesInput.setAttribute(
                      "name",
                      `niveles_${gesValue.replace(/[^a-zA-Z0-9]/g, "_")}`
                    );
                    needsAppendNiveles = true;
                    console.log(
                      `      Input hidden PREPARADO para niveles: ${gesValue}`
                    );
                  } else {
                    console.log(
                      `      Input hidden ENCONTRADO para niveles: ${gesValue}`
                    );
                  }
                  ...
                  console.log(
                    `      >> Valor JSON ASIGNADO a input (niveles): ${nivelesInput.value}`
                  );
                  ...
                  if (needsAppendNiveles) {
                    infoGeneralSection.appendChild(nivelesInput);
                    console.log(
                      `      Input hidden AÑADIDO al DOM para niveles`
                    );
                  }

                  // Verificación INMEDIATA
                  const verifyNiveles = infoGeneralSection.querySelector(
                    `input[type="hidden"][data-riesgo="${gesValue}"][data-niveles]`
                  );
                  if (
                    verifyNiveles &&
                    verifyNiveles.value === valorJsonGuardado
                  ) {
                    console.log(
                      `      ✅ Verificación INMEDIATA OK para niveles. Valor JSON en DOM: "${verifyNiveles.value}"`
                    );
                  } else {
                    console.error(
                      `      ❌ Verificación INMEDIATA FALLÓ para niveles. Input:`,
                      verifyNiveles
                    );
                  }
                } else {
                  // Fin if(infoGeneralSection)
                  console.error(
                    `      Error: No se encontró .info-general-section para niveles en ${gesValue} DENTRO del cargo ${currentCargoNameForLog}`
                  );
                }
              } else {
                // Fin if(gesData.niveles)
                console.log(`    No hay datos de niveles para ${gesValue}`);
              }
            } else {
              // Fin if(checkbox)
              console.warn(
                `    Checkbox no encontrado en DOM para: ${gesValue} dentro del cargo ${currentCargoNameForLog}`
              );
            }
          }); // Fin forEach gesData
          ...
          console.log(
            `  Actualizando resumen final para ${currentCargoNameForLog}`
          );
          updateGesResumen(cargoEl);
        } else {
          // Fin if(cargoData.gesSeleccionados)
          console.log(
            `  ${currentCargoNameForLog} no tiene GES seleccionados.`
          );
          // Asegurarse que el resumen muestre "No hay GES" si estaba vacío
          updateGesResumen(cargoEl);
        }
      }); // Fin forEach cargoData

      // Ajustar botón de eliminar si solo queda un cargo
      const allCargos = cargoContainer.querySelectorAll(".cargo");
      console.log(
        `Restauración completada. Total de cargos en DOM: ${allCargos.length}`
      );
      if (allCargos.length === 1) {
        const deleteBtn = allCargos[0].querySelector(".delete-btn");
        if (deleteBtn) {
          deleteBtn.disabled = true;
          console.log("Botón de eliminar del único cargo deshabilitado.");
        }
      }

      // Marcar tutorial como visto si se restauró al menos un cargo
      ...
      alert(
        "Ocurrió un error al restaurar los datos guardados. Se cargará un formulario nuevo."
      );
      ...
      // Asegurarnos que la verificación se ejecute incluso si hay errores leves
      ...
      setTimeout(() => {
        ...
        const cargosDespuesRestauracion =
          cargoContainer.querySelectorAll(".cargo");
        console.log(
          `Verificando ${cargosDespuesRestauracion.length} cargos después de restauración...`
        );
        ...
        cargosDespuesRestauracion.forEach((cargoVerificar, index) => {
          const cargoName =
            cargoVerificar.querySelector('input[name="cargoName"]')?.value ||
            `(Cargo sin nombre ${index + 1})`;
          console.log(`\n--- VERIFICANDO CARGO ${index + 1}: ${cargoName} ---`);

          const infoGeneral = cargoVerificar.querySelector(
            ".info-general-section"
          );
          if (!infoGeneral) {
            console.error(
              "  ¡Error crítico! No se encontró .info-general-section en este cargo."
            );
            return; // No podemos verificar más si falta esto
          }

          const gesCheckboxes = cargoVerificar.querySelectorAll(
            'input[type="checkbox"][name^="ges_cargo_"]:checked'
          );
          console.log(
            `  GES seleccionados encontrados: ${gesCheckboxes.length}`
          );

          // Obtener TODOS los inputs hidden de control/nivel DENTRO de infoGeneral para este cargo
          const todosLosHiddenEnInfo = infoGeneral.querySelectorAll(
            'input[type="hidden"][data-riesgo]'
          );
          console.log(
            `  Total inputs hidden [data-riesgo] en .info-general: ${todosLosHiddenEnInfo.length}`
          );
          todosLosHiddenEnInfo.forEach((inp) => {
            const tipo = inp.dataset.tipo
              ? `Control ${inp.dataset.tipo}`
              : inp.dataset.niveles
              ? "Niveles"
              : "Desconocido";
            console.log(
              `    Input encontrado: ${tipo} para ${inp.dataset.riesgo}, Valor="${inp.value}"`
            );
          });
          ...
          gesCheckboxes.forEach((checkbox) => {
            ...
            ["fuente", "medio", "individuo"].forEach((tipo) => {
              ...
              const valorEsperado = state.cargos[index]?.gesSeleccionados?.find(
                (g) => `${g.riesgo} - ${g.ges}` === gesValue
              )?.controles?.[tipo];

              if (controlInput) {
                console.log(
                  `    ✅ Control ${tipo}: Existe. Valor DOM="${
                    controlInput.value
                  }". (Esperado: "${valorEsperado || ""}")`
                );
                if (controlInput.value !== (valorEsperado || "")) {
                  console.warn(
                    `      ¡Advertencia! El valor restaurado (${
                      controlInput.value
                    }) no coincide con el esperado (${valorEsperado || ""})`
                  );
                }
              } else {
                ...
                if (
                  ...
                  console.error(
                    `    ❌ Control ${tipo}: ¡INPUT HIDDEN FALTANTE! (Esperado: "${valorEsperado}")`
                  );
                } else {
                  console.log(
                    `    Control ${tipo}: No existe (y no se esperaba valor).`
                  );
                }
              }
            });
            ...
            const nivelesEsperadosObj = state.cargos[
              index
            ]?.gesSeleccionados?.find(
              (g) => `${g.riesgo} - ${g.ges}` === gesValue
            )?.niveles;
            const nivelesEsperadosJson = nivelesEsperadosObj
              ? JSON.stringify(nivelesEsperadosObj)
              : undefined;
            ...
            if (!nivelesInput) {
              ...
              if (nivelesEsperadosJson && nivelesEsperadosJson !== "{}") {
                console.error(
                  `    ❌ Niveles: ¡INPUT HIDDEN FALTANTE! (Esperado JSON: ${nivelesEsperadosJson})`
                );
              } else {
                console.log(
                  `    Niveles: No existe (y no se esperaban niveles).`
                );
              }
            } else {
              console.log(
                `    ✅ Niveles: Existe. Valor JSON en DOM="${
                  nivelesInput.value
                }". (Esperado JSON: ${nivelesEsperadosJson || "{}"})`
              );
              if (nivelesInput.value !== (nivelesEsperadosJson || "{}")) {
                ...
                try {
                  ...
                  if (JSON.stringify(domObj) !== JSON.stringify(expectedObj)) {
                    console.warn(
                      `      ¡Advertencia! El valor JSON restaurado no coincide exactamente con el esperado.`
                    );
          ...
          todosLosHiddenEnInfo.forEach((inp) => {
            if (!gesMarcados.includes(inp.dataset.riesgo)) {
              const tipo = inp.dataset.tipo
                ...
                : "Desconocido";
              console.warn(
                `  Input "huérfano" encontrado: ${tipo} para ${inp.dataset.riesgo} (GES no está marcado). Valor="${inp.value}"`
              );
            }
          });
        ...
        console.log(
          "🔧 === FIN VERIFICACIÓN POST-RESTAURACIÓN (Delayed) ===\n"
        );
  ...
  if (
    estadoGuardado &&
    estadoGuardado.cargos &&
    estadoGuardado.cargos.length > 0
  ) {
  ...
  addCargoBtn.addEventListener("click", () => {
    ...
    if (todosLosCargos.length === 1) {
      const deleteBtnPrimerCargo =
        todosLosCargos[0].querySelector(".delete-btn");
...
// ==================== FIN CÓDIGO MODAL ====================
  ...
  window.debugNiveles = function () {
    ...
    cargos.forEach((cargo, index) => {
      ...
      if (infoGeneralSection) {
        const inputsEnInfoGeneral = infoGeneralSection.querySelectorAll(
          "[data-niveles], [data-tipo]"
        );
        console.log(
          `  Inputs hidden en info-general: ${inputsEnInfoGeneral.length}`
        );
        ...
        inputsEnInfoGeneral.forEach((input) => {
          if (input.dataset.niveles) {
            ...
            console.log(
              `    CONTROL ${input.dataset.tipo}: ${input.dataset.riesgo} = ${input.value}`
            );
          }
        });
      }
      ...
      const inputsFueraDeInfo = cargo.querySelectorAll(
        "[data-niveles], [data-tipo]"
      );
      const inputsEnInfo = infoGeneralSection
        ? infoGeneralSection.querySelectorAll("[data-niveles], [data-tipo]")
        : [];
      console.log(`  Total inputs hidden: ${inputsFueraDeInfo.length}`);
      console.log(`  Inputs en info-general: ${inputsEnInfo.length}`);
      console.log(
        `  Inputs fuera de info-general: ${
          inputsFueraDeInfo.length - inputsEnInfo.length
        }`
      );
      ...
      const gesSeleccionados = cargo.querySelectorAll(
        'input[type="checkbox"][name^="ges_cargo_"]:checked'
      );
    ...
    try {
      ...
      if (savedData) {
        ...
        parsed.cargos.forEach((cargoGuardado, index) => {
          ...
          console.log(
            `  GES guardados: ${cargoGuardado.gesSeleccionados.length}`
          );
    ...
    if (savedData) {
      ...
      if (domCargos === storedCargos) {
        ...
        console.warn(
          `❌ Cantidad de cargos no coincide: DOM(${domCargos}) vs Stored(${storedCargos})`
        );
      }
      ...
      cargos.forEach((cargo, index) => {
        ...
        console.log(
          `Cargo ${index + 1}: DOM tiene ${
            nivelesInputs.length
          } niveles, localStorage tiene ${gesGuardados} GES`
        );
      });
    }
  ...
  window.debugRestauracionVisual = function () {
    ...
    cargos.forEach((cargo, index) => {
      ...
      gesSeleccionados.forEach((checkbox) => {
        ...
        const inputNiveles = cargo.querySelector(
          `[data-riesgo="${gesValue}"][data-niveles]`
        );
        if (inputNiveles) {
          ...
          try {
            ...
            ["deficiencia", "exposicion", "consecuencia"].forEach(
              (tipoNivel) => {
                if (niveles[tipoNivel]) {
                  console.log(
                    `     ${tipoNivel}: ${niveles[tipoNivel].label} (${niveles[tipoNivel].value})`
                  );
                } else {
        ...
        if (gesResumen) {
          const itemResumen = Array.from(
            gesResumen.querySelectorAll(".ges-resumen-item")
          ).find((item) => item.textContent.includes(gesValue));
          if (itemResumen) {
    ...
    console.log(
      "2. Observa los logs de showControlesPopup y getNivelSeleccionado"
    );
  ...
  console.log(
    "Funciones de debugging disponibles: debugNiveles(), forzarGuardado(), debugRestauracionVisual(), forzarRecreacionInputs(), forzarActualizacionIconos()"
  );
  ...
  window.forzarRecreacionInputs = function () {
    ...
    cargos.forEach((cargo, index) => {
      ...
      const gesCheckboxes = cargo.querySelectorAll(
        'input[type="checkbox"][name^="ges_cargo_"]:checked'
      );
      ...
      gesCheckboxes.forEach((checkbox) => {
        ...
        let inputNiveles = cargo.querySelector(
          `[data-riesgo="${gesValue}"][data-niveles]`
        );
        ...
        if (!inputNiveles) {
          ...
          try {
            ...
            if (savedData) {
              ...
              if (cargoGuardado) {
                const gesGuardado = cargoGuardado.gesSeleccionados.find(
                  (ges) => `${ges.riesgo} - ${ges.ges}` === gesValue
                );

                if (
                  gesGuardado &&
                  gesGuardado.niveles &&
                  Object.keys(gesGuardado.niveles).length > 0
                ) {
                  console.log(`🔧 Recreando con niveles:`, gesGuardado.niveles);

                  const infoGeneralSection = cargo.querySelector(
                    ".info-general-section"
                  );
                  if (infoGeneralSection) {
  ...
  window.forzarActualizacionIconos = function () {
    ...
    cargos.forEach((cargo, index) => {
      ...
      gesResumenItems.forEach((item) => {
        ...
        console.log(
          `   ${icon} ${gesText} - ${isComplete ? "COMPLETO" : "INCOMPLETO"}`
        );
      });
    });
